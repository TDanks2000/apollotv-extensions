...
class Anilist extends MediaProvier {
  ...
  private findAnimeSlug = async (
    ...
    if (possibleAnime.subOrDub) {
      if (
        possibleAnime.subOrDub != SubOrDubOrBoth.BOTH &&
  ...
  private findMangaSlug = async (
    ...
    if (possibleSource) {
      possibleManga = await provider.getMediaInfo(
  ...
  private findMangaRaw = async (
    ...
    if (!possibleManga) {
      return (await provider.getMediaInfo(
