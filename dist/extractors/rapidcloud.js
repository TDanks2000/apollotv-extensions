...
class RapidCloud extends types_1.VideoExtractor {
    constructor() {
        super(...arguments);
        ...
        this.extract = (videoUrl) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                //   });
                // }
                res = yield this.client.get(`https://${videoUrl.hostname}/embed-2/ajax/e-1/getSources?id=${id}`, options);
                let { data: { sources, tracks, intro, encrypted }, } = res;
                let decryptKey = yield (yield this.client.get("https://github.com/enimax-anime/key/blob/e6/key.txt")).data;
                decryptKey = (0, utils_1.substringBefore)((0, utils_1.substringAfter)(decryptKey, '"blob-code blob-code-inner js-file-line">'), "</td>");
                if (!decryptKey) {
                    decryptKey = yield (yield this.client.get("https://raw.githubusercontent.com/enimax-anime/key/e6/key.txt")).data;
                }
                if (!decryptKey)
                ...
                if (videoUrl.href.includes(new URL(this.host).host)) {
                    ...
                    for (const source of sources) {
                        ...
                        const secondHalf = m3u8data.map((line) => {
                            var _a;
                            return (_a = line
                                .match(/RESOLUTION=.*,(C)|URI=.*/g)) === null || _a === void 0 ? void 0 : _a.map((s) => s.split("=")[1]);
                        });
        ...
        this.captcha = (url, key) => __awaiter(this, void 0, void 0, function* () {
            ...
            const { data } = yield this.client.get(`https://www.google.com/recaptcha/api.js?render=${key}`, {
                headers: {
                    Referer: domain,
                },
            });
            ...
            const c = (0, cheerio_1.load)((yield this.client.get(anchor)).data)("#recaptcha-token").attr("value");
            // currently its not returning proper response. not sure why
            const res = yield this.client.post(`https://www.google.com/recaptcha/api2/reload?k=${key}`, {
                v: v,
                k: key,
                c: c,
